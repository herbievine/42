...
/*   Updated: 2022/11/15 08:43:04 by herbie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int ft_parse_special_chars(char c, void *arg)
{
	...
	if (c == 'p')
		return (ft_putptr_fd((long long int)arg, 1));
	if (c == 'd' || c == 'i')
		return (ft_putnbr_fd((int)arg, 1));
	if (c == 'u')
		return (ft_putunbr_fd((unsigned int)arg, 1));
	if (c == 'x')
		return (ft_puthex_fd((long long int)arg, 1));
	if (c == 'X')
		return (ft_puthex_up_fd((long long int)arg, 1));
	if (c == '%')	
